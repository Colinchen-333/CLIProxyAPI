...
One CLIProxy process owns all self-hosted model routing. A dedicated Node
transport retains the original official Claude TLS/HTTP behavior and identity
proxy; no official Claude requests are used for testing. The regular API remains on its
configured address; the optional loopback split listener routes official Claude
through the original Node identity transport and dispatches registered own models into the
same API handler in memory. There is no Node routing layer on the own-provider path, local upstream socket pool,
...
split-relay:
  ...
  official-relay-url: http://127.0.0.1:8319
  max-request-bytes: 67108864
```

The relay port above is local-only. The Node transport resolves its existing
identity proxy from the existing identity configuration; it does not change
accounts, user-agent, cookies, timezone, or exit selection. CLI flags
`--split-listen` and `--split-official-relay` override
...
installing this topology. Only `ai.lumirain.claudex-cliproxy` owns the API and mixed-session addresses.
`ai.lumirain.claude-official-transport` owns the local official connector port;
it runs `claude-official-transport` and the accompanying `.mjs` file under
launchd, never as a background shell job.
...
Credential synchronization is a separate short-lived launchd job named
`ai.lumirain.cliproxy-auth-sync`, running the existing
`~/.local/bin/cliproxy-sync-opencode-go`. `RunAtLoad` and `WatchPaths` on
`~/.local/share/opencode/auth.json` refresh the generated configuration, which
the gateway already hot-reloads. No network credential validation blocks the
gateway listener; failures leave their diagnostic in the synchronization log.

The Node connector is copied from the pre-existing official CONNECT/TLS/request
functions. Its test uses a local HTTPS fixture behind a local CONNECT proxy,
with a temporary test CA; it sends nothing to an official service. Transport
profiles beyond these retained code paths have not been tested against Claude.
